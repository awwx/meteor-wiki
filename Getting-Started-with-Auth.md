...
- `sendVerificationEmail` (Boolean) - Send email address verification emails to users created via `accounts-password`.
...
- [Client] `Accounts.verifyEmail(token, callback)` - Verify a user's email
 - `token`: unique string contained in the email sent to the user by a client-side call to `Accounts.createUser` (in case `sendVerificationEmail` was set to true in the call to `Accounts.config`)
 - `callback`: Function(error|null)
